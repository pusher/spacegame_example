...
Pusher.prototype.createAuthToken = function(channel_name, socket_id, channel_data) {
  var secret_key = this.options.secret_key,
      public_key = this.key,
      sig_data = [socket_id, channel_name];
...
Pusher.authorizers = {
  node: function(pusher, callback) {
    var response = pusher.createAuthToken(this.name, pusher.socket_id, pusher.options.channel_data);
    callback(response);
  }
};
...
})(module, require('websocket-client').WebSocket);